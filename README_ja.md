...
- セッショントークンは**デバイスフィンガープリントに紐づけ**されます（Session Key はセッション確立のたびに動的にローテーションされ、更新・プロキシ要求は紐づけられたフィンガープリントと一致する必要があります）。トークンはサーバー側で HMAC 署名されるため、フロントエンドは他セッションを偽造・復号できません。
- フロントエンド唯一のインライン注入点（エラーバナー）は**コンテキストに応じた出力エンコード**のホワイトリスト描画へ収束し、静的ページと `/api` レスポンスの双方に **CSP**（インライン・外部スクリプト禁止、オブジェクト埋め込み禁止）を適用して XSS を多層防御します。
