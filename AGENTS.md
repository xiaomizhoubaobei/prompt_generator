...
> 本节在 `2.2` 既有"中文注释"要求之上，补充面向 TypeScript 项目的统一文档注释规范。**所有代码注释与文档字符串必须使用中文编写**，专有名词、变量名与关键字保持英文原文。

#### 2.4.1 文件级文档字符串
每个新文件必须在文件头（许可证与版权信息之后）添加文件级文档字符串，简要说明该文件的核心职责与包含的主要类/函数。
 ...
 * 用户服务模块。
 *
 * 该文件负责处理与用户相关的所有业务逻辑，包括用户创建、查询、更新和删除。
 * 导出的主要函数包括 createUser、getUserById、updateUser。
 */
```
 ...
 * @param {boolean} includeProfile - 是否同时返回用户的详细资料，默认为 false。
 * @returns {Promise} 返回一个包含用户信息的 Promise 对象。
 * @throws {UserNotFoundError} 当根据 userId 找不到对应用户时抛出此异常。
 * @throws {DatabaseConnectionError} 当数据库连接失败时抛出此异常。
 */
async function getUserById(userId: string, includeProfile = false): Promise<User> {
  // 函数实现
}
```
