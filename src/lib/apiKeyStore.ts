 ...
 * @remark
 * 本模块用于在纯前端架构下承载用户填写的第三方 API Key，且不在客户端留下任何
 * 可被提取的密钥/凭据。
 *
 ...
 * - 不在客户端代码中回退到任何 VITE_* 环境变量：Vite 会把 VITE_* 变量替换进
 *   bundle，构建期注入的 Key 对每个能下载应用的用户都仍可提取，故不再作为兜底；
 * - 页面刷新后内存清空，用户需在设置中重新填写 Key；
 * - 保留既有的 apiUrl / modelName 等非敏感配置持久化能力。
 *
 * 对外接口：
 * - getApiKey(): 读取当前内存中的 API Key
...
/**
 * 读取当前 API Key
 * 仅返回本次运行期用户通过设置写入的 Key；内存为空时返回空字符串。
 *
