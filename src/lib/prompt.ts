/**
 * @fileoverview 提示词生成模块，提供多种提示词框架的模板生成功能
 * @author 祁筱欣
 * @date 2026-02-11
 * @since 2026-02-11
 ...
 *          使用方式：
 *          - 导入 handlePrompt 函数
 *          - 传入框架类型 key、任务 task 和可选的自定义指令 customText
 *          - 返回对应框架的提示词模板
 *
 *          示例：
 *          - const prompt = handlePrompt('CO-STAR', '写一篇宣传AI的小红书')
 */
/**
 * 提示词生成函数
 * 根据指定的框架类型生成对应的提示词模板
 *
 * @param key - 提示词框架类型（CO-STAR、CRISPE、DRAW、Meta Prompting、CoT、VARI、Q*、RISE、MicrOptimization、O1-STYLE、OpenAI、claude、Custom）
 * @param task - 用户输入的任务描述
 * @param customText - 自定义提示词优化指令（仅用于 Custom 框架）
 * @returns 生成的提示词模板字符串
 */
