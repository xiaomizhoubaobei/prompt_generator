/**
 * @fileoverview 主应用组件，提供提示词生成和优化功能
 * @author 祁筱欣
 * @date 2026-02-11
 * @since 2026-02-11
 ...
 *          - 提示词更新：支持对已有提示词进行迭代优化
 *          - 图片分析：支持上传图片生成描述提示词
 *          - 测试功能：支持测试生成的提示词效果
 *          - 多语言支持：支持中文、英文、日文
 *          - 自定义指令：允许用户自定义提示词优化指令
 *
 *          支持的提示词框架：
 *          - CO-STAR：新加坡Prompt大赛冠军框架
 *          - CRISPE：基于LangGPT框架的工作流拆解
 *          - DRAW：绘画提示词生成（镜头、光线、主体、背景、风格、氛围）
 *          - Meta Prompting：清华大学和上海AI实验室提出的方法
 *          - CoT：思维链提示
 *          - VARI：Google Deepmind变分规划
 *          - Q*：马尔可夫决策过程优化
 *          - RISE：卡内基梅隆大学递归内省
 *          - MicrOptimization：微软自动优化
 *          - O1-STYLE：模仿o1的结构化思考
 *          - OpenAI：OpenAI官方方法
 *          - claude：Claude官方方法
 *          - Custom：自定义提示词优化指令
...
function App(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    if (selectedModel) { dispatch(setGlobalState({ selectedModel })) }
  }, [])
  ...
  useEffect(() => {
    ...
    if (savedSettings) {
      try {
        ...
        if (parsed.modelName) {
          dispatch(setGlobalState({ selectedModel: parsed.modelName }))
        }
      } catch (e) {
  ...
  /**
   * 生成提示词函数
   * 根据用户输入的任务内容和选择的提示词框架，生成结构化的提示词
   */
  const onGeneratePromptFunc = async () => {
    ...
    return proxyChat(body).then(async (response) => {
      ...
      setErrComp(await responseHandler.getErrorMessage(error, global.language));
      setIsDisabled(false);
      setIsUpd(false);
      console.log(error);
    })
  ...
  /**
   * 判断是否显示图片上传功能
   * 根据当前模型是否支持图片生成来决定是否显示上传按钮
   * 返回是否显示图片上传功能
   */
  const onDisplayUpload = (): boolean => {
    const model = global.selectedModel || import.meta.env.VITE_APP_MODEL_NAME;
   ...
   * 返回自定义提示词指令对话框组件
   */
  const onCustom = (): React.JSX.Element => {
    ...
    <div className="absolute top-0 left-0 right-0 flex flex-col min-h-full bg-[#f5f5f5]" >
      {
        Boolean(errComp) && (
          ...
          >
            ...
            <div><ErrorRenderer html={errComp} /></div>
          </div>
        )
      }
      < div className="flex justify-end p-2 box-border" >
        <div className='flex absolute right-10 top-2 gap-3'><SettingsDialog /><LanguagePopover /></div>
      </div >
